            ...
            if($post_error === 0){
                 ...
                 header("Location: create_acc.php?img_size=false"); #this will execute if the file ist too large
                     ...
                     if(in_array($img_ex_lc, $allowed_exs)) {
                       ...
                       $img_upload_path = 'posts/post_pics/' . $new_post_name;
             ...
             if(isset($_SESSION['admin_status'])){
                $post_stmt = $conn->prepare("INSERT INTO `senior_system`.`post_tbl` (`emp_id`, `event_type_id`, `post_description`, `post_pic`, `post_date`, `post_time`, `date_created`, `time_created`, `post_loc`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $post_stmt->bind_param("issssssss", $_SESSION['emp_id'], $event_type_id, $post_desc, $new_post_name, $post_date, $post_time, $date_created, $time_created, $post_loc);
                $post_stmt->execute();
